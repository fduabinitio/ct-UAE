...
from ct.data_mt_2 import CIFData
from ct.data_mt_2 import collate_pool, get_train_val_test_loader,collate_pool_train
from ct.model_mt_2 import CrystalTransformer
from sklearn.metrics import r2_score
import random
from torch.optim.lr_scheduler import CosineAnnealingLR
os.environ["WANDB_API_KEY"] = '84ca8c6c885fe483ff1668004110693b2c7f5749'

warnings.filterwarnings("ignore", category=UserWarning, module="pymatgen")

parser = argparse.ArgumentParser(description='Crystal Graph Convolutional Neural Networks')

parser.add_argument('--load', default=r'mp_13')
parser.add_argument('--file',default='mp_13_2_fb.csv')
...
def main():
    ...
    if args.task == 'classification':
        ...
        _, sample_target1,sample_target2, _ = collate_pool(sample_data_list)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, normalizer1,normalizer2)

        # evaluate on validation set
        mae_error,r2_score,mae_error2,r2_score2 = validate(val_loader, model, criterion, normalizer1,normalizer2)
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer1,normalizer2):
    ...
    for i, (input, target1,target2, _) in enumerate(train_loader):
...
def validate(val_loader, model, criterion, normalizer1,normalizer2 ,test=False):
    batch_time = AverageMeter()
    losses1 = AverageMeter()
    losses2 = AverageMeter()
    if args.task == 'regression':
    ...
    for i, (input, target1,target2, batch_cif_ids) in enumerate(val_loader):
        if args.cuda:
            with torch.no_grad():
                input_var = (Variable(input[0].to(args.device)),
                             Variable(input[1].to(args.device)),
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'model_best_formation_256.pth.tar')
