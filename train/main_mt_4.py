...
from ct.data_mt_4 import CIFData
from ct.data_mt_4 import collate_pool, get_train_val_test_loader,collate_pool_train
from ct.model_mt_4 import CrystalTransformer
...
def main():
    ...
    if args.task == 'classification':
        ...
        _, sample_target1,sample_target2,sample_target3,sample_target4, _ = collate_pool(sample_data_list)
        normalizer1 = Normalizer(sample_target1)
        normalizer2 = Normalizer(sample_target2)
        normalizer3 = Normalizer(sample_target3)
        normalizer4 = Normalizer(sample_target4)
    # build model
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, normalizer1,normalizer2,normalizer3,normalizer4)

        # evaluate on validation set
        mae_error,r2_score,mae_error2,r2_score2 = validate(val_loader, model, criterion, normalizer1,normalizer2,normalizer3,normalizer4)
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer1,normalizer2,normalizer3,normalizer4):
    ...
    for i, (input, target1,target2,target3,target4, _) in enumerate(train_loader):
        ...
        if args.task == 'regression':
            ...
            target_normed4 = normalizer4.norm(target4)
        else:
        ...
        if args.cuda:
            ...
            target_var4 = Variable(target_normed4.to(args.device))
        else:
        ...
        loss4 = criterion(output4, target_var4)
...
def validate(val_loader, model, criterion, normalizer1,normalizer2,normalizer3,test=False):
    ...
    for i, (input, target1,target2,target3,target4, batch_cif_ids) in enumerate(val_loader):
        if args.cuda:
            with torch.no_grad():
                input_var = (Variable(input[0].to(args.device)),
                             Variable(input[1].to(args.device)),
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'model_best_mt4_256.pth.tar')
